...
function addImplicitVerses(lines){let result=[],added=0,i=0;while(i<lines.length){if(!lines[i].trim()){result.push(lines[i++]);continue}let j=i;while(j<lines.length&&lines[j].trim())j++;const block=lines.slice(i,j),hasSection=block.some(isSection),hasChord=block.some(isLikelyChordLine),hasLyric=block.some(line=>line.trim()&&!isSection(line)&&!isLikelyChordLine(line));if(!hasSection&&hasChord&&hasLyric&&added<2){result.push('[Verse :]');added++}result.push(...block);i=j}return result}
