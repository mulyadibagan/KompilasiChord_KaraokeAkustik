...
function removeCodeFences(lines){return lines.filter(line=>!/^\s*\x60{3,}\s*$/.test(line))}
function trimBlankLines(lines){let start=0,end=lines.length;while(start<end&&!lines[start].trim())start++;while(end>start&&!lines[end-1].trim())end--;return lines.slice(start,end)}
function expandReturnInstructions(lines){const marker=lines.findIndex(line=>/^\s*\\?\*\)\s*$/.test(line));if(marker<0)return lines.filter(line=>!/^\s*\\?\*\)\s*$/.test(line));const reff=lines.findIndex((line,index)=>index>marker&&canonicalSection(line)==='[Reff :]');if(reff<0)return lines.filter(line=>!/^\s*\\?\*\)\s*$/.test(line));let reffEnd=lines.findIndex((line,index)=>index>reff&&!!canonicalSection(line));if(reffEnd<0)reffEnd=lines.length;const starBlock=trimBlankLines(lines.slice(marker+1,reff)),reffBlock=trimBlankLines(lines.slice(reff,reffEnd));let out=[];for(const line of lines){if(/^\s*\\?\*\)\s*$/.test(line))continue;if(/^\s*\\?=+\s*kembali\s+ke\s*:\s*\\?\*\)\s*,\s*reff(?:rain)?\s*$/i.test(line)){if(out.length&&out[out.length-1].trim())out.push('');out.push(...starBlock,'',...reffBlock);continue}out.push(line)}return out}
...
function expandChordRepeats(lines){let expanded=[];for(const line of lines){const match=line.match(/^(.*?)\s*([\[(])\s*(\d+)\s*x\s*[\])]\s*$/i);if(!match){expanded.push(line);continue}const progression=match[1].replace(/\s+$/,''),bracket=match[2],count=Number(match[3]);if(!progression.trim()||!Number.isSafeInteger(count)||count<1||!isLikelyChordLine(progression)){expanded.push(line);continue}if(bracket==='['){for(let i=0;i<count;i++)expanded.push(progression);continue}for(let remaining=count;remaining>0;remaining-=2)expanded.push(remaining>1?progression+' '+progression.trim():progression)}return expanded}
...
function formatBody(body){let lines=splitInputLines(body);lines=removeCodeFences(lines);lines=expandChordRepeats(lines);lines=removeMisplacedVerse2(lines);lines=removeIndentedVerse2(lines);lines=addImplicitVerses(lines);lines=normalizeSections(lines);lines=lines.map(replaceEllipsisPreserveColumns);lines=lines.map(normalizeChordNotation);lines=lines.map(line=>isLikelyChordLine(line)?normalizeTransitionChordLine(line):line);lines=normalizeChordLyricPairs(lines);lines=expandReturnInstructions(lines);return lines.map(x=>x.replace(/\s+$/,'')).join('\n').replace(/\n{3,}/g,'\n\n').trim()}
