...
function normalizeChordNotation(line){return line.replace(/\((-?[A-G](?:#|b)?(?:m|maj|min|dim|aug|sus|add)?\d*(?:\/[A-G](?:#|b)?)?)\)/g,'$1').replace(/\s*\(\s*\d+\s*x\s*\)\s*$/i,'')}
...
function formatBody(body){let lines=splitInputLines(body);lines=expandChordRepeats(lines);lines=removeMisplacedVerse2(lines);lines=addImplicitVerses(lines);lines=normalizeSections(lines);lines=lines.map(replaceEllipsisPreserveColumns);lines=lines.map(normalizeChordNotation);lines=lines.map(line=>isLikelyChordLine(line)?normalizeTransitionChordLine(line):line);let inReff=false;for(let i=0;i<lines.length;i++){const sec=canonicalSection(lines[i]);if(sec){inReff=/^\[Reff(?:\s+Overtone)?\s*:\]$/i.test(sec);continue}if(!inReff||!isLikelyChordLine(lines[i]))continue;let j=i+1;while(j<lines.length&&lines[j].trim()==='')j++;if(j<lines.length&&!isSection(lines[j])&&!isLikelyChordLine(lines[j]))[lines[i],lines[j]]=shiftReffPair(lines[i],lines[j])}return lines.map(x=>x.replace(/\s+$/,'')).join('\n').replace(/\n{3,}/g,'\n\n').trim()}
