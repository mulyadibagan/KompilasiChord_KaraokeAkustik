...
function normalizeReturnInstructions(lines){const returnsToStar=lines.some(line=>/^\s*\\?=+\s*kembali\s+ke\s*:\s*\\?\*\)/i.test(line));return lines.map(line=>{if(returnsToStar&&/^\s*\\?\*\)\s*$/.test(line))return'[Pre-Reff :]';const match=line.match(/^\s*\\?=+\s*kembali\s+ke\s*:\s*(.*?)\s*$/i);if(!match)return line;const targets=match[1].split(',').map(item=>item.trim().replace(/^\\/,'')).filter(Boolean).map(item=>/^\*\)$/.test(item)?'Pre-Reff':/^reff(?:rain)?$/i.test(item)?'Reff':item);return'[Kembali ke : '+targets.join(', ')+']'})}
function removeStandaloneMarkers(lines){return lines.filter(line=>!/^\s*(?:\\?\*\)|\x60{3,})\s*$/.test(line))}
