...
function splitInputLines(body){const inline=/^\s*\[?((?:reff\s+overtone|pre[- ]?(?:reff|chorus)|post[- ]?(?:reff|chorus)|intro|musik|interlude|reff(?:rain)?|chorus|overtone|bridge|outro|verse\s*\d*))\s*:\]?\s*(.*?)\s*$/i;let expanded=[];for(const line of body.replace(/\r/g,'').split('\n')){const match=line.match(inline);if(!match){expanded.push(line);continue}expanded.push(canonicalSection(match[1]+':'));if(match[2])expanded.push(match[2])}return expanded}
...
function formatBody(body){let lines=splitInputLines(body);lines=normalizeSections(lines);lines=lines.map(line=>isLikelyChordLine(line)?normalizeTransitionChordLine(line):replaceEllipsisPreserveColumns(line));let inReff=false;for(let i=0;i<lines.length;i++){const sec=canonicalSection(lines[i]);if(sec){inReff=/^\[Reff(?:\s+Overtone)?\s*:\]$/i.test(sec);continue}if(!inReff||!isLikelyChordLine(lines[i]))continue;let j=i+1;while(j<lines.length&&lines[j].trim()==='')j++;if(j<lines.length&&!isSection(lines[j])&&!isLikelyChordLine(lines[j]))[lines[i],lines[j]]=shiftReffPair(lines[i],lines[j])}return lines.map(x=>x.replace(/\s+$/,'')).join('\n').replace(/\n{3,}/g,'\n\n').trim()}
function rootOf(token){let t=token.replace(/^-+/,'').split(/\s+/)[0],m=t.match(/^([A-G](?:#|b)?)(m?)/);return m?m[1]+m[2]:''}
function detectKey(body){let lines=splitInputLines(body),intro=false,candidates=[];for(const line of lines){const sec=canonicalSection(line);if(sec){if(sec==='[Intro :]'){intro=true;continue}if(intro)break;continue}if(intro&&isLikelyChordLine(line))for(const p of line.trim().split(/\s+/)){let r=rootOf(p);if(r)candidates.push(r)}}if(!candidates.length)for(const line of lines)if(isLikelyChordLine(line)){let r=rootOf(line.trim().split(/\s+/)[0]);if(r){candidates.push(r);break}}if(!candidates.length)return'';let first=candidates[0],last=candidates[candidates.length-1];if(first===last)return first;let counts={};for(const c of candidates)counts[c]=(counts[c]||0)+1;return Object.entries(counts).sort((a,b)=>b[1]-a[1])[0][0]||first}
