...
function canonicalSection(line){let t=line.trim().replace(/^\[|\]$/g,'').trim();if(/^intro\s*:?$/i.test(t))return'[Intro :]';if(/^(?:musik|interlude)\s*:?$/i.test(t))return'[Interlude :]';if(/^(?:reff(?:rain)?|chorus)\s*:?$/i.test(t))return'[Reff :]';if(/^(?:reff\s+overtone|overtone)\s*:?$/i.test(t))return'[Reff Overtone :]';if(/^pre[- ]?(?:reff|chorus)\s*:?$/i.test(t))return'[Pre-Reff :]';if(/^post[- ]?(?:reff|chorus)\s*:?$/i.test(t))return'[Post-Reff :]';if(/^bridge\s*:?$/i.test(t))return'[Bridge :]';if(/^outro\s*:?$/i.test(t))return'[Outro :]';let v=t.match(/^verse\s*(\d+)?\s*:?$/i);return v?('[Verse'+(v[1]?' '+v[1]:'')+' :]'):null}
function splitInputLines(body){const inline=/^\s*\[?((?:reff\s+overtone|pre[- ]?(?:reff|chorus)|post[- ]?(?:reff|chorus)|intro|musik|interlude|reff(?:rain)?|chorus|overtone|bridge|outro|verse\s*\d*))\s*:\]?\s*(.*?)\s*$/i;let expanded=[],inlineOffset=0;for(const line of body.replace(/\r/g,'').split('\n')){const match=line.match(inline);if(match){expanded.push(canonicalSection(match[1]+':'));if(match[2]){inlineOffset=line.indexOf(match[2]);expanded.push(match[2])}else inlineOffset=0;continue}if(!line.trim()){expanded.push(line);inlineOffset=0;continue}if(inlineOffset){const lead=(line.match(/^ */)||[''])[0].length;expanded.push(line.slice(Math.min(lead,inlineOffset)));continue}expanded.push(line)}return expanded}
function isSection(line){return !!canonicalSection(line)}
function isLikelyChordLine(line){const t=line.replace(/\.{2,}/g,m=>' '.repeat(m.length)).trim();if(!t||isSection(t))return false;const parts=t.split(/\s+/);return parts.length>0&&parts.every(p=>chordToken.test(p))}
...
function addImplicitVerses(lines){let result=[],seenReff=false,i=0;const sections=lines.map(canonicalSection).filter(Boolean),hasVerse1=sections.includes('[Verse 1 :]'),hasVerse2=sections.includes('[Verse 2 :]');let verse1Added=hasVerse1,verse2Added=hasVerse2;while(i<lines.length){if(!lines[i].trim()){result.push(lines[i++]);continue}let j=i;while(j<lines.length&&lines[j].trim())j++;const block=lines.slice(i,j),blockSections=block.map(canonicalSection).filter(Boolean),hasSection=blockSections.length>0,hasChord=block.some(isLikelyChordLine),hasLyric=block.some(line=>line.trim()&&!isSection(line)&&!isLikelyChordLine(line));if(!hasSection&&hasChord&&hasLyric){if(!seenReff&&!verse1Added){result.push('[Verse 1 :]');verse1Added=true}else if(seenReff&&!verse2Added){result.push('[Verse 2 :]');verse2Added=true}}result.push(...block);if(blockSections.some(sec=>/^\[Verse(?:\s+\d+)?\s*:\]$/i.test(sec))){if(seenReff)verse2Added=true;else verse1Added=true}if(blockSections.some(sec=>/^\[Reff(?:\s+Overtone)?\s*:\]$/i.test(sec)))seenReff=true;i=j}return result}
function shiftReffPair(chord,lyric){const lyricLead=(lyric.match(/^ */)||[''])[0].length;if(!lyricLead)return[chord,lyric];const chordLead=(chord.match(/^ */)||[''])[0].length;const shift=Math.min(lyricLead,chordLead);return[chord.slice(shift),lyric.slice(shift)]}
function formatBody(body){let lines=splitInputLines(body);lines=removeMisplacedVerse2(lines);lines=addImplicitVerses(lines);lines=normalizeSections(lines);lines=lines.map(replaceEllipsisPreserveColumns);lines=lines.map(line=>isLikelyChordLine(line)?normalizeTransitionChordLine(line):line);let inReff=false;for(let i=0;i<lines.length;i++){const sec=canonicalSection(lines[i]);if(sec){inReff=/^\[Reff(?:\s+Overtone)?\s*:\]$/i.test(sec);continue}if(!inReff||!isLikelyChordLine(lines[i]))continue;let j=i+1;while(j<lines.length&&lines[j].trim()==='')j++;if(j<lines.length&&!isSection(lines[j])&&!isLikelyChordLine(lines[j]))[lines[i],lines[j]]=shiftReffPair(lines[i],lines[j])}return lines.map(x=>x.replace(/\s+$/,'')).join('\n').replace(/\n{3,}/g,'\n\n').trim()}
function rootOf(token){let t=token.replace(/^-+/,'').split(/\s+/)[0],m=t.match(/^([A-G](?:#|b)?)(m?)/);return m?m[1]+m[2]:''}
function detectKey(body){for(const line of splitInputLines(body)){const cleaned=line.replace(/\.{2,}/g,m=>' '.repeat(m.length));if(isLikelyChordLine(cleaned)){const root=rootOf(cleaned.trim().split(/\s+/)[0]);if(root)return root}}return''}
